...
function showToast(message, type = "success") {
  ...
  el.className =
    "toast-in flex items-center gap-2.5 bg-white border border-line shadow-lg rounded-lg pl-3 pr-4 py-2.5 text-sm font-medium text-ink";
  el.innerHTML = `
...
function applyView() {
  ...
  document
    .getElementById("statCards")
    .parentElement.querySelector("#statCards").style.display =
...
function openModal(task = null) {
  ...
  document.getElementById("fDue").value =
    task && task.dueDate ? task.dueDate.substring(0, 10) : "";
...
function renderRow(task) {
  ...
  row.innerHTML = `
    ...
    <div class="flex items-center gap-2 shrink-0 ml-auto sm:ml-0">
      ${
        overdue
          ? `<span class="text-[11px] font-semibold text-white px-2.5 py-1 rounded-full shrink-0 whitespace-nowrap bg-high flex items-center gap-1">

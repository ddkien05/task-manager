...
function isOverdue(t) {
  return !!t.dueDate && !t.completed && new Date(t.dueDate) < startOfToday();
}
...
function applyTheme(mode) {
  ...
  document
    ...
    .forEach((el) => el.classList.toggle("hidden", mode === "dark"));
  document
...
function showToast(message, type = "success") {
  const colors = { success: "#22B07D", error: "#EF4B4B", info: "#2F6FED" };
  const icon =
    type === "error"
      ? '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><path d="M12 8v5M12 16h.01"/></svg>'
      : '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5"><path d="M20 6L9 17l-5-5"/></svg>';
  const toast = document.createElement("div");
  toast.className =
    "flex items-center gap-2 text-white text-sm font-medium px-4 py-2.5 rounded-lg shadow-lg fade-in";
...
function setAuthMode(mode) {
  ...
  tabLogin.classList.toggle("text-muted", !isLogin);
  ...
  tabRegister.classList.toggle("text-muted", isLogin);
  authSubmit.textContent = isLogin ? "Đăng nhập" : "Tạo tài khoản";
...
authForm.addEventListener("submit", async (e) => {
  ...
  try {
    const res = await fetch(
      `${AUTH_API}/${authMode === "login" ? "login" : "register"}`,
      {
        method: "POST",
    ...
    authError.textContent = "Không thể kết nối tới máy chủ";
    authError.classList.remove("hidden");
  } finally {
...
function applyView() {
  ...
  document.getElementById("calendarPanel").style.display =
    state.view === "settings" ? "none" : "";
  document
    .getElementById("prioFilters")
    .classList.toggle("hidden", state.view !== "tasks");
  document
    .getElementById("prioFilters")
    .classList.toggle("flex", state.view === "tasks");
  document
    .getElementById("openAdd")
    .classList.toggle(
      "hidden",
      state.view === "settings" || state.view === "archive",
    );
  refreshPrioUI();
  loadTasks();
}
...
async function checkHealth() {
  ...
  try {
    ...
    dot.className =
      "w-2 h-2 rounded-full shrink-0 " + (ok ? "bg-low" : "bg-high");
    label.textContent = ok ? "Running" : "Disconnected";
    label.className =
      "text-xs font-semibold " + (ok ? "text-low" : "text-high");
    dotM.className =
      "w-2 h-2 rounded-full shrink-0 " + (ok ? "bg-low" : "bg-high");
    labelM.textContent = ok ? "Running" : "Disconnected";
    labelM.className =
      "text-xs font-semibold whitespace-nowrap " +
      (ok ? "text-low" : "text-high");
  } catch {
...
function openModal(task = null, prefillDate = null) {
  ...
  document.getElementById("fDue").value =
    task && task.dueDate ? task.dueDate.substring(0, 10) : prefillDate || "";
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    closeModal();
    showToast(
      isEdit ? "Đã cập nhật công việc" : "Đã thêm công việc",
      "success",
    );
    loadTasks();
  } catch (err) {
...
function openDetail(task) {
  ...
  document.getElementById("detailCreated").textContent = formatDateTime(
    task.createdAt,
  );
  document.getElementById("detailUpdated").textContent = formatDateTime(
    task.updatedAt,
  );
...
dayBackdrop.addEventListener("click", (e) => {
  if (e.target === dayBackdrop) closeDayModal();
});
...
document.getElementById("calPrev").addEventListener("click", () => {
  state.calMonth = new Date(
    state.calMonth.getFullYear(),
    state.calMonth.getMonth() - 1,
    1,
  );
  loadCalendar();
});
...
async function loadCalendar() {
  try {
    const res = await fetch(API); // toàn bộ task của user hiện tại, không lọc theo view
...
function renderCalendarGrid(tasks) {
  ...
  const kindOf = (t) =>
    t.completed ? "done" : isOverdue(t) ? "overdue" : "doing";
...
function renderAgenda(tasks) {
  ...
  const upcoming = pending
    .filter((t) => !isOverdue(t) && t.dueDate)
    .sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate))
    .slice(0, 6);
...
function renderRow(task) {
  ...
  row.className =
    "flex flex-wrap sm:flex-nowrap items-center gap-x-3 gap-y-2 py-3 fade-in cursor-pointer hover:bg-canvas/60 rounded-lg px-1 -mx-1 transition-colors";
  ...
  row.innerHTML = `
    ...
    <div class="flex items-center gap-2 shrink-0 ml-auto sm:ml-0">
      ${overdue ? '<span class="text-[11px] font-semibold text-white bg-high px-2.5 py-1 rounded-full shrink-0 whitespace-nowrap">Quá hạn</span>' : ""}
  ...
  row.querySelector(".toggleBtn").addEventListener("click", (e) => {
    e.stopPropagation();
    toggleTask(task._id);
  });
...
async function toggleTask(id) {
  try {
    ...
    showToast(
      task.completed ? "Đã hoàn thành công việc" : "Đã chuyển về đang làm",
